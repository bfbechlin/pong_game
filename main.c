...
int main(){
    ...
    PADDLE pad = {.position.x = MAP_WIDTH/2, .position.y = MAP_HEIGHT-2, .velocity.x = 0, .velocity.y =0, .len = 15, .vertical = FALSE, .advanceKey = KEY_LEFT, .regressKey = KEY_RIGHT};
    LEVEL level = {.dificult = 1, .nBall =1};
    FRAME frameGame = {.height = MAP_HEIGHT, .width = MAP_WIDTH};
    
    // Gerando a seed para o RANMAR que será usada durante todo o tempo de jogo
    seedGen(); 
